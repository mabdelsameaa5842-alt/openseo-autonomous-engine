...
export class OfficeState {
  ...
  /** Walk a character to specific tile coordinates */
  walkCharacterTo(id: number, targetCol: number, targetRow: number): boolean {
    const ch = this.characters.get(id);
    if (!ch) return false;
    const path = findPath(ch.tileCol, ch.tileRow, targetCol, targetRow, this.tileMap, this.blockedTiles);
    if (path.length > 0) {
      ch.path = path;
      ch.moveProgress = 0;
      ch.state = CharacterState.WALK;
